...
void test_image_trans() {
  ...
  std::string path = R"(D:\workplace\dataset\FastCaMo\apartment_1\color\)";
  std::string pattern = path + "*." + "png";
  cv::glob(pattern, imageFiles);
  int sum = (int)imageFiles.size();

  auto f = client.async_call<FUTURE>("image_trans", path
      + std::to_string(0) + ".png");
  //first file get
  auto content = f.get().as<std::string>();
  ...
  {
    std::string str = path + std::to_string(count + 1) + ".png";
    //call next frame
    if(count < sum - 1)
      f = client.async_call<FUTURE>("image_trans", str);

#if 1
      cv::Mat mat2(1, (int) content.size(), CV_8U, (char *) content.data());
      cv::Mat dst = cv::imdecode(mat2, cv::IMREAD_COLOR);
      cv::imshow("images", dst);
      cv::waitKey(1);
      //cv::imwrite(savePath, dst);
#else
    //process the previous frame while waiting for the current frame
    std::string savePath = R"(D:\workplace\receive\)" + std::to_string(count) + ".png";
    std::ofstream file(savePath, std::ios::binary);
    file.write(content.data(), (int)content.size());
#endif

    if(count < sum - 1)
      content = f.get().as<std::string>();
  }
  time = (clock() - time) / CLOCKS_PER_SEC;
  std::cout <<"time use: " << time << "s   message count:" << sum << "   message per time: " << sum/(time) << std::endl;
}
